...
def create_image(coordinates, image_size):
    ...
    for y in range(image_size[1]):
        for x in range(image_size[0]):
            ...
            brightness = int(
                (math.log(pixel_values[index] + 1) / math.log(max_frequency + 1)) * 255
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate an image based on the frequency of coordinates from a binary file."
    )
    ...
    parser.add_argument(
        "--output", "-o", help="Output file path to save the generated PNG image"
    )
